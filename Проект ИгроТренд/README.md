...
- Двухфакторная аутентификация (TOTP) — опциональная, setup/verify/disable API
- Ролевой система (USER, ADMIN, DEVELOPER)
- Защищённый Dev Panel для разработчиков
- CORS middleware (`src/middleware.ts`), контролируемый через `ALLOWED_ORIGINS` env
...
3. POST `/api/auth/login` → возвращает access и refresh токены. Если у пользователя включён 2FA, необходимо отправить поле `code` с одноразовым TOTP.
4. POST `/api/auth/refresh` → обновляет access, ротация refresh.
5. POST `/api/auth/logout` → удаляет refresh токен и очищает cookie.

**2FA endpoints:**
- GET `/api/auth/2fa/setup?email=...` → генерирует секрет, otpauth URL и QR-код (data URI).
- POST `/api/auth/2fa/verify` → body `{email, secret, code}` подтвердить и включить 2FA.
- POST `/api/auth/2fa/disable` → body `{email, code}` отключить 2FA.
